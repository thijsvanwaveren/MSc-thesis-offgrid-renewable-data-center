...
def extract_economic_results():
    """
    Reads all Feasible_3D_Sweep_Results_99.9pct_IT*.csv files, filters for Tier A >= 8 MW, 
    and applies the master thesis 'Hero Plot' macroeconomic calculation engine to find
    the Annualized Net Profit.
    """
    ...
    # --- Financial Inputs from Hero Plot Script ---
    ...
    for file in files:
        ...
        # STAGE 2: Apply the Hero Plot Calculation Engine to every remaining row
        
        # 1. Calculate Gross Revenue in Euros
        # Energy C = Total Delivered - (A + B1 + B2)
        ...
        filtered_df['Net_Profit_M'] = (filtered_df['Gross_Rev_EUR'] - filtered_df['Water_OPEX_EUR']) / 1e6 - HPP_SUNK_COST_M - total_fixed_costs_m
        ...
        tier_a = best_row['Tier_A_MW']
        tier_b1 = best_row['Tier_B1_MW']
        tier_b2 = best_row['Tier_B2_MW']
        
        delivered_gwh = best_row['Total_Delivered_Annual_GWh']
        revenue_m = best_row['Gross_Rev_EUR'] / 1e6
        profit_m = best_row['Net_Profit_M']

        # I am keeping LCOED calculation based on the previous prompt for context, renamed to "Electricity Cost"
        electricity_cost_m = (delivered_gwh * 1000 * best_row['LCOE_delivered']) / 1e6

        results.append({
            'IT (MW)': capacity_mw,
            'Mix (A/B1/B2)': f"{int(tier_a)}/{int(tier_b1)}/{int(tier_b2)}",
            'Delivered (GWh)': round(delivered_gwh, 1),
            'Gross Rev (€M)': round(revenue_m, 2),
            'Fixed Cost (€M)': round(total_fixed_costs_m, 2),
            'Elec Cost (€M)': round(electricity_cost_m, 2),
            'Net Profit (€M)': round(profit_m, 2)
        })
    ...
    print("\n--- Data Center Master Economic Overview (With Hero Plot Calculation Engine) ---")
