# -*- coding: utf-8 -*-
"""
Created on Tue Mar 24 15:00:03 2026
...
def run_iterative_search():
    ...
    MAX_IT_CAPACITY_MW = 550.0  # <--- Strict Hardware Cap
    N_life_1yr = 8760
    ...
    # =========================================================================
    # 1. PROFILES & STRICT CAPACITY SETUP
    # =========================================================================
    ...
    # NEW: Create a flat 35 MW array to force the EMS to recognize the hard cap
    fixed_capacity_profile = np.full(N_life_total, MAX_IT_CAPACITY_MW)

    # =========================================================================
    # 2. INITIALIZE OPENMDAO
    # =========================================================================
    print(f"\nInitializing OpenMDAO Model with strict {MAX_IT_CAPACITY_MW} MW limit...")
    hpp_yearly = hpp_model(
        ...
        load_profile_ts=fixed_capacity_profile, # <--- Passthrough to enforce IT capacity
        battery_deg=False,
        run_mode='yearly' 
    )
    ...
    for b1_mw in range(1, 101): 
        tier_b1_25yr = np.full(N_life_total, b1_mw * 24.0)
        
        hpp_yearly.prob.set_val('tier_b_profile', tier_b1_25yr)
        
        # We NO LONGER update load_profile_ts here. It stays glued at MAX_IT_CAPACITY_MW.
    ...
    print(f"  Firm Capacity Blocks : Variable (from CSV, scaled 0.95)")
    print(f"  Tier B1 (Daily)      : {max_b1_mw} MW Constant")
    print(f"  Tier B2 (Weekly)     : {max_b2_mw} MW Constant")
    print(f"  IT Capacity Cap      : {MAX_IT_CAPACITY_MW} MW")
    print("="*60 + "\n")
