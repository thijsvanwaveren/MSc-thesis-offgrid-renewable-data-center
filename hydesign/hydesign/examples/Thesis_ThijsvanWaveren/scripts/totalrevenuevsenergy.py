# -*- coding: utf-8 -*-
"""
Section 3.4 - The Revenue Story (Corrected Pareto Frontier)
Plots Gross Revenue vs. Committed Energy Delivered.
Calculates a perfectly smooth upper envelope by finding the 
maximum revenue across exact energy intervals.
"""
...
REVENUE_PER_MWH = {"A": 3000.0, "B1": 2100.0, "B2": 1200.0}
...
# 2. Calculate Gross Revenue (Y-Axis)
...
# =============================================================================
print("Generating Scatter Plot and Revenue Frontier...")
...
# and find the absolute maximum Y (Revenue) inside each column.
df['Energy_Rounded'] = df["Energy_Delivered_No_C_GWh"].round(0)
idx_max = df.groupby('Energy_Rounded')["Revenue_No_C_M_EUR"].idxmax().dropna()
...
ax.plot(upper_bound["Energy_Delivered_No_C_GWh"], 
        upper_bound["Revenue_No_C_M_EUR"], 
...
ax.set_ylabel("Gross Revenue [Millions € / Year]", fontweight='bold', color='#595959', fontsize=12)
...
legend_elements = [
    ...
    Line2D([0], [0], color='#e74c3c', lw=2.5, label='Efficiency Frontier (Max Revenue)'),
...
# Save the plot
save_path = os.path.join(BASE_FOLDER, f"Thesis_Revenue_vs_Energy_{IT_CAPACITY}MW.SVG")
