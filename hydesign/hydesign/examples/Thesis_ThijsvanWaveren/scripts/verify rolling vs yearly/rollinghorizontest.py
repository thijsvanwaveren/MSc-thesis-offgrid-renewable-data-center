# -*- coding: utf-8 -*-
"""
Rolling Horizon EMS with Centralized Unified Penalties (Latency Included)
"""
...
# ==============================================================================
# GLOBAL PENALTY WEIGHTS (The "DNA" of the Solver)
# ==============================================================================
PENALTIES = {
    'VoLL_Tier_A': 1.0e5,          
    'VoLL_Tier_B1': 1.0e3,         
    'VoLL_Tier_B2': 1.0e2,         
    'Cost_Queue_Latency': 1.0e-1,  
    'Cost_Curtail': 5.0e-1,        
    'Cost_Deg': 1.0e-1,            
    'Reward_C2': -0.5              
}
...
class ems_constantoutput(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if batt_eff < 0.01: batt_eff = 0.923
        ...
        if self.run_mode == 'rolling':
            # FIXED: Catching Q_B1_ts and Q_B2_ts from the rolling wrapper
            (P_HPP_ts, P_curt_ts, P_batt_ts, E_SOC_ts,
             Unserved_A_ts, Shortfall_B_ts, Shortfall_B2_ts, Served_C2_ts, Q_B1_ts, Q_B2_ts) = solve_rolling(
            ...
            # FIXED: Latency penalty included
            penalty_ts = (Unserved_A_ts * PENALTIES['VoLL_Tier_A']) + \
                         (Shortfall_B_ts * PENALTIES['VoLL_Tier_B1']) + \
                         (Shortfall_B2_ts * PENALTIES['VoLL_Tier_B2']) + \
                         (Q_B1_ts * PENALTIES['Cost_Queue_Latency']) + \
                         (Q_B2_ts * PENALTIES['Cost_Queue_Latency']) + \
                         (Served_C2_ts * PENALTIES['Reward_C2'])
            ...
            print("   ... CPLEX solving 8760 hours (PERFECT FORESIGHT) ...")
...
def ems_cplex_window(
    ...
    obj  = mdl.sum(U_tier_A[t] for t in time) * PENALTIES['VoLL_Tier_A']
    obj += mdl.sum(V_B1_t[t] for t in time) * PENALTIES['VoLL_Tier_B1']
    obj += mdl.sum(Q_B1_t[t] for t in time) * PENALTIES['Cost_Queue_Latency']
    obj += mdl.sum(V_B2_t[t] for t in time) * PENALTIES['VoLL_Tier_B2']
    obj += mdl.sum(Q_B2_t[t] for t in time) * PENALTIES['Cost_Queue_Latency']
    obj += mdl.sum(P_curtail_t[t] for t in time) * PENALTIES['Cost_Curtail']
    obj += mdl.sum(P_charge[t] + P_discharge[t] for t in time) * PENALTIES['Cost_Deg']
    obj += mdl.sum(P_tier_C2[t] for t in time) * PENALTIES['Reward_C2']
    ...
    if sol is None or not mdl.solve_details.status.startswith('opt'):
        print("⚠️ WARNING: CPLEX Window Infeasible or Failed!")
...
def solve_rolling(
    ...
    S_B2   = np.zeros(T)
    P_C2   = np.zeros(T)
    Q_B1   = np.zeros(T) # FIXED: Added missing array
    Q_B2   = np.zeros(T) # FIXED: Added missing array
    E_soc  = np.zeros(T + 1)
    ...
    while t < T:
        ...
        Q_B1[sl]   = Q_B1_win[:h_apply] # FIXED: Saving queue backlogs
        Q_B2[sl]   = Q_B2_win[:h_apply] # FIXED: Saving queue backlogs
    ...
    return P_HPP, P_curt, out_P_batt, E_soc, U_A, S_B1, S_B2, P_C2, Q_B1, Q_B2 # FIXED: Returning arrays
...
def ems_cplex_offgrid_full_horizon(
    ...
    obj_A = mdl.sum(U_tier_A[t] for t in time) * PENALTIES['VoLL_Tier_A']
    obj_B_Deadline = mdl.sum(Missed_Deadline_B_t[t] for t in time) * PENALTIES['VoLL_Tier_B1']
    obj_B_Latency = mdl.sum(Queue_B_t[t] for t in time) * PENALTIES['Cost_Queue_Latency']
    obj_B2_Deadline = mdl.sum(Missed_Deadline_B2_t[t] for t in time) * PENALTIES['VoLL_Tier_B2']
    obj_B2_Latency = mdl.sum(Queue_B2_t[t] for t in time) * PENALTIES['Cost_Queue_Latency'] 
    obj_Curt = mdl.sum(P_curtailment_t[t] for t in time) * PENALTIES['Cost_Curtail']
    obj_Deg = mdl.sum((P_charge[t] + P_discharge[t]) for t in time) * PENALTIES['Cost_Deg']
    obj_C2 = mdl.sum(P_tier_C2[t] for t in time) * PENALTIES['Reward_C2']
    ...
    Q_B1_res = extract(Queue_B_t)           # FIXED: Extracted queue
    Q_B2_res = extract(Queue_B2_t)          # FIXED: Extracted queue

    # FIXED: Latency penalty included
    Penalty_res = (U_A_res * PENALTIES['VoLL_Tier_A']) + \
                  (S_B_res * PENALTIES['VoLL_Tier_B1']) + \
                  (S_B2_res * PENALTIES['VoLL_Tier_B2']) + \
                  (Q_B1_res * PENALTIES['Cost_Queue_Latency']) + \
                  (Q_B2_res * PENALTIES['Cost_Queue_Latency']) + \
                  (P_C2_res * PENALTIES['Reward_C2'])
