# -*- coding: utf-8 -*-
"""
Created on Tue Feb  3 14:36:25 2026
...
## --- DEMAND PROFILES ---------------------------------------------------
# 1. Update Path to your specific folder
base_path = r"C:\Users\thijs\Downloads\hydesign\hydesign\examples\Thesis_ThijsvanWaveren\inputs"

# 2. Define ONLY your single profile
target_filename = "DataCenter_Constant_Load.csv" 
profile_name_clean = target_filename.replace('.csv', '')

profiles_info = {
    target_filename: "DataCenter"
}

# 3. Simplify Categories
category_profiles = {
    "DataCenter": [profile_name_clean]
}

# 4. Initialize Storage
load_profiles_MW = {}       
load_profiles_scaled_MW = {} 
profile_categories = {profile_name_clean: "DataCenter"}
...
for filename, category in profiles_info.items():
    ...
    try:
        # FIX: Added delimiter=';' back. 
        # Using index_col=0 means the first column (Date) becomes the Index.
        df = pd.read_csv(full_path, index_col=0, delimiter=';', parse_dates=True)
        
        # Debugging: Print shape to confirm it worked
        print(f"Reading {filename}... Shape: {df.shape}")
        
        if df.shape[1] == 0:
            raise ValueError("File loaded but no data columns found. Check if delimiter is ',' instead of ';'")

        # Take the FIRST available data column (regardless of name)
        # 0.001 converts kW -> MW
        load_ts_MW = pd.to_numeric(df.iloc[:, 0], errors='coerce').dropna() * 0.001
        
        # Validation
        print(f"✅ Loaded {filename}: Mean = {load_ts_MW.mean():.2f} MW, Peak = {load_ts_MW.max():.2f} MW")

        # Store Data (No Scaling)
        load_profiles_MW[profile_name_clean] = load_ts_MW
        load_profiles_scaled_MW[profile_name_clean] = load_ts_MW 

    except Exception as e:
        print(f"❌ Error loading {filename}: {e}")
        # Stop script so you can fix the error before running a massive loop
        break
...
## --- DESIGN PARAMETERS -----------------------------------------------
Nwt_list = [3, 6, 9, 12, 15]             #[7, 8, 10, 14, 16]         #[9, 11, 13, 15, 18]         #[3, 6]  #[3, 6, 9, 12, 15] # , 12, 15, 18, 21, 24]                                 #[3, 6, 9, 12, 15, 18] 
b_P_list = [40, 50, 60, 70, 80]     #[5, 35, 45] #60, 70, 80]              # [35, 45, 55, 65, 75]                #[20, 30] #[30, 40, 50, 60, 70] # 60, 70, 80, 90] 
solar_MW_list =[40, 50, 60, 70 ,80] #[35, 45, 55] #[70, 80, 90]        #[40, 55, 65, 75, 85]            #[25, 35] #[35, 45, 55, 65, 75] #[100, 110,120, 130, 140]                         #[25, 35, 45, 55, 65]
...
for category, profile_list in category_profiles.items():
    for profile_name in profile_list:
        ...
        ## -- LCOE --------------------------------------------------------------------------
            ...
            over_load_sum = np.sum(over_load) 
            wind_WACC= 0.052                                                          # Markup of after tax WACC for onshore WT
            solar_WACC= 0.048                                                         # After tax WACC for solar PV
            battery_WACC= 0.080                                                       # After tax WACC for stationary storge li-ion batteries
            ...
            AEPy_scalar = hpp_t_first_year.sum() 
            print(f"Total Energy Delivered (AEPy_scalar) = {AEPy_scalar:,.0f} MWh") 
            ...
            # level_costs / CL likewise if you export them as column

            C_WIND      = hpp.prob.get_val('finance.CAPEX_w').item()
            O_WIND      = hpp.prob.get_val('finance.OPEX_w').item()
            C_PV        = hpp.prob.get_val('finance.CAPEX_s').item()
            O_PV        = hpp.prob.get_val('finance.OPEX_s').item()
            C_BATT      = hpp.prob.get_val('finance.CAPEX_b').item()
            O_BATT      = hpp.prob.get_val('finance.OPEX_b').item()
            C_EL        = hpp.prob.get_val('finance.CAPEX_el').item()
            O_EL        = hpp.prob.get_val('finance.OPEX_el').item()
            level_costs = hpp.prob.get_val('finance.level_costs').item()

            # 4) sum to get scalars
            C_H = C_WIND + C_PV + C_BATT + C_EL
            O_H = O_WIND + O_PV + O_BATT + O_EL

            results[profile_name]["FLF"].append(CF_cplex)
            results[profile_name]["LCOE"].append(LCOE.item())
            ...
            summary_records.append({
                ...
                "C_Wind": round(C_WIND, 2),
                "O_wind": round(O_WIND, 2),
                "C_PV": round(C_PV, 2),
                "O_PV": round(O_PV, 2),
                "C_BATT": round(C_BATT, 2),
                "O_BATT": round(O_BATT, 2),
                "C_EL": round(C_EL, 2),
                "O_EL": round(O_EL, 2),
                "C_H": round(C_H, 2),
                "O_H": round(O_H, 2),
                "CL": round(level_costs, 2),
                "over_load": round(over_load_sum, 2),
            })
...
num = df_all.select_dtypes(include="number").columns
df_all[num] = df_all[num].astype(float).round(2)
...
os.makedirs("excel_per_profile_batt4h_86efficiency_Denmark_good_solar", exist_ok=True)
...
for profile in df_all["Profile"].unique():
    ...
    excel_df = df_sorted[[
        ...
        "FLF_%",
        "ULF_%",
        "C_Wind", "O_wind", "C_PV", "O_PV",
        "C_BATT", "O_BATT", "C_EL", "O_EL",
        "C_H", "O_H", "CL", "over_load",
    ]]

    # Bestandsnaam maken zonder spaties of speciale tekens
    safe_profile_name = profile.replace(" ", "_").replace("/", "-")
    file_path = f"excel_per_profile_batt4h_86efficiency_Denmark_good_solar/add_{safe_profile_name}.xlsx"
 ...
 #%%   
# Maak map aan om bestanden op te slaan (optioneel)
os.makedirs("excel_per_profile_4h_86efficiency_Denmark_good_solar", exist_ok=True)
...
for profile in df_all["Profile"].unique():
    ...
    # 4) write it out
    safe_profile_name = profile.replace(" ", "_").replace("/", "-")
    file_path = f"excel_per_profile_4h_86efficiency_Denmark_good_solar/add_{safe_profile_name}.xlsx"
    excel_df.to_excel(file_path, index=False)
    print(f"Excel file saved for profile: {profile} → {file_path}")
...
# Maak map aan om bestanden op te slaan (optioneel)
os.makedirs("excel_per_profile_4h_second_86efficiency_Denmark_good_solar", exist_ok=True)
...
for profile in df_all["Profile"].unique():
    ...
    # Bestandsnaam maken zonder spaties of speciale tekens
    safe_profile_name = profile.replace(" ", "_").replace("/", "-")
    file_path = f"excel_per_profile_4h_second_86efficiency_Denmark_good_solar/add_{safe_profile_name}.xlsx"
