...
curtailment and unutilized hardware, normalized as resource inefficiency.
"""
...
# Plot the Main Curves (Added white marker edges for publication-quality crispness)
ax.plot(IT_CAPACITIES_MW, curtailment_pct, color=C_CURTAIL, marker='o', markersize=9, 
        ...
        label='System Inefficiency (HPP Curtailment)', zorder=4)

ax.plot(IT_CAPACITIES_MW, unutilized_pct, color=C_UNUTIL, marker='s', markersize=9, 
        markeredgecolor='white', markeredgewidth=1.5, linewidth=2.5, 
        label='Hardware Inefficiency (Unutilized IT Capacity)', zorder=4)

# =============================================================================
# 3. FORMATTING
# =============================================================================
# Refined, punchy Y-axis title
ax.set_ylabel("Resource Inefficiency (%)", fontsize=12, fontweight='bold', color='#222222')
ax.set_xlabel("Installed IT Capacity (MW$_{IT}$)", fontsize=12, fontweight='bold', color='#222222')
...
ax.set_ylim(0, max(curtailment_pct.max(), unutilized_pct.max()) * 1.05)
ax.yaxis.set_major_formatter(mtick.PercentFormatter(decimals=0))

# Clean, professional grid and spines
ax.grid(axis='y', linestyle='-', alpha=0.2, color='#b0b0b0', zorder=0)
ax.grid(axis='x', linestyle='-', alpha=0.2, color='#b0b0b0', zorder=0)
