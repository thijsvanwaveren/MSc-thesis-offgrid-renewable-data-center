...
def plot_step_1_baseline():
    ...
    ax.stackplot(time_index, res_base['wind'], res_base['solar'], colors=[C_WIND, C_SOLAR], labels=['Wind Generation', 'Solar Generation'], alpha=0.8, zorder=3)
    ax.plot(time_index, res_base['gen'], color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
...
def plot_step_2_firm_storage():
    ...
    ax1.plot(time_index, res_a_only['gen'], color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
...
def plot_step_3_flexible_queue():
    ...
    ax1.plot(time_index, res_b1_only['gen'], color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
    ax1.stackplot(time_index, b_base, b_catch, colors=[C_B1, C_B1_CATCH], labels=['Tier B1 (10 MW)', 'Tier B1 Catch-up'], alpha=0.9, zorder=3)
...
def plot_step_4_full_hierarchy():
    ...
    ax1.stackplot(time_index, res_base['s_a'], b_base, b_catch, colors=[C_A, C_B1, C_B1_CATCH], labels=['Tier A', 'Tier B1', 'Tier B1 Catch-up'], alpha=0.9, zorder=3)
    ax1.plot(time_index, res_base['gen'], color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
...
def plot_step_5_b1battery():
    ...
    ax1.stackplot(time_index, res_c['s_a'], b_base, b_catch, res_c['s_c'], 
                  colors=[C_A, C_B1, C_B1_CATCH, C_C], labels=['Tier A', 'Tier B1', 'Tier B1 Catch-up', 'Tier C (Opportunistic)'], alpha=0.9, zorder=3)
    ax1.plot(time_index, res_c['gen'], color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
    ax1.axhline(MAX_IT_MW, color=C_LIMIT, linestyle='-.', linewidth=1.0, alpha=0.8, zorder=5, label='Hardware Limit')
...
def plot_step_6_b1b2():
    ...
    ax1.stackplot(time_index, b1_b, b2_b, b1_c, b2_c, 
                  ...
                  labels=['Tier B1 (8 MW)', 'Tier B2 (8 MW)', 'B1 Catch-up', 'B2 Catch-up'], 
                  alpha=0.9, zorder=3)
    
    ax1.plot(time_index, res_dual['gen'], color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
...
def plot_step_7_firm_fail():
    ...
    # -------------------------------------------------------------------------
    ax1.plot(time_index, gen, color=C_GEN, linewidth=1.5, linestyle='--', zorder=6, label='Total Generation')
    ax1.fill_between(time_index, 0, s_a, facecolor=C_A, alpha=0.9, zorder=3, label='Served Tier A')
