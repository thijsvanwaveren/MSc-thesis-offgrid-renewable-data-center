...
# # p=1
# print("Loading from:", load_profile)
# print(load_profile_ts.head(10))
# print("unique values:", np.unique(load_profile_ts.values)[:5])
...
class finance(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        #df['hpp_t']=  inputs['hpp_t'] # without degradation
        ...
        print(df[['hpp_t','load_profile_ts']].head(24))
        print("unique load_profile_ts values (first 10):",
            np.unique(df['load_profile_ts'])[:10])
        print("length of load_profile_ts:", len(df['load_profile_ts']))
        print("---------------------------")
        ...
        AEP_per_year = (
            df
            .assign(
                hpp_t_meet = df['load_profile_ts']
                                .where(df['hpp_t'] >= df['load_profile_ts'], 0)
            )
            .groupby('i_year')
            .hpp_t_meet
            .mean()
        ...
        print('AEP_per_year', AEP_per_year)

        for t in [0,1,2,3,4,5]:
            print(t,
                "hpp_t=", df.at[t,'hpp_t'],
                "demand=", df.at[t,'load_profile_ts'])
        print('hpp_discount_factor', hpp_discount_factor)
        level_AEP = np.sum(AEP_per_year / (1 + hpp_discount_factor)**iy)
        print("level_AEP", level_AEP)
