...
class ems_PPA_EM(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if self.ems_type == 'cplex':
            ems_WSB = ems_cplex_PPA_EM
        else:
            raise Warning("This class should only be used for ems_cplex_PPA_EM")
...
def ems_cplex_PPA_EM(
    ...
    for ib, batch in enumerate(batches):
        ...
        E_SOC_ts_batch, penalty_batch = ems_cplex_parts_PPA_EM(
            wind_ts = wind_ts_sel,
...
def ems_cplex_parts_PPA_EM(
    ...
    P_HPP_PPA_t = mdl.continuous_var_dict(
        time, lb=0, ub=hpp_grid_connection, 
        name='HPP PPA power output')
    P_HPP_EM_t = mdl.continuous_var_dict(
        time, lb=0, ub=hpp_grid_connection, 
        name='HPP PPA power output')
    ...
    for t in time:
        ...
        mdl.add_constraint(
            P_HPP_PPA_t[t] + P_HPP_EM_t[t] == wind_ts[t] +
            solar_ts[t] +
            - P_curtailment_t[t] +
