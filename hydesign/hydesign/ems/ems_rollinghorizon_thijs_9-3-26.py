...
class ems_constantoutput(om.ExplicitComponent):
    ...
    # Rolling-horizon (recommended for weekly/realistic operation)
        ...
        # Annual perfect-foresight baseline (your existing code)
...
def ems_cplex_window(
    ...
    it_capacity_mw,
    # --- rolling-horizon cross-window state:
    soc_init,                              # float, MWh
    qB_init, qB2_init,                     # float, MWh (initial backlogs)
    prev_arrivals_B1, prev_arrivals_B2,    # arrays len 23 / 167 (MWh), arrival memory
    # --- end-of-window guidance (soft terminal SoC):
    soc_target=None, w_terminal_soc=0.0,
    # --- cplex perf tuning:
    threads=4, timelimit=120, mipgap=0.001
):
    assert prev_arrivals_B1.shape == (23,)
    assert prev_arrivals_B2.shape == (167,)
    assert len(wind_ts) == len(solar_ts) == len(tier_a) == len(tier_b_daily) == len(tier_b2_weekly)
    ...
    mdl.context.cplex_parameters.lpmethod = 4
    ...
    for i, t in enumerate(time):
        ...
        # FIXED: Map the battery state transition flawlessly across the entire array
        tt = SOCtime[i+1]
        mdl.add_constraint(
    ...
    # OBJECTIVE
    obj  = mdl.sum(U_tier_A[t] for t in time) * 1.0e8
    obj += mdl.sum(V_B1_t[t] for t in time) * 1.0e3
    obj += mdl.sum(Q_B1_t[t] for t in time) * 1.0e-4
    obj += mdl.sum(V_B2_t[t] for t in time) * 1.0e2
    obj += mdl.sum(Q_B2_t[t] for t in time) * 1.0e-4
    obj += mdl.sum(P_curtail_t[t] for t in time) * 1.0e-2
    obj += mdl.sum(P_charge[t] + P_discharge[t] for t in time) * 1.0e-1
    ...
    # FIXED: The only extraction method that guarantees absolute chronological alignment
...
def solve_rolling(
    ...
    while t < T:
        ...
        A_win  = tier_a[t:t_end]
        B1_win = tier_b_daily[t:t_end]
        B2_win = tier_b2_weekly[t:t_end]
    ...
    return P_HPP, P_curt, P_batt, E_soc, U_A, S_B1, S_B2, P_C2
