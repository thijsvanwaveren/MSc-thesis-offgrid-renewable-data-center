...
class ems_constantoutput(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        # Determine IT Capacity from the input profile max (or assume it matches load peak)
        # We assume the user constructed Tier A + Tier B such that max(Tier A + B) = Capacity
        # But since B is flexible, we must constrain it. 
        # We'll calculate total capacity from the provided load profile's max value or passed param.
        # Ideally, this should be an explicit input, but we can infer it from the max load requested.
        ...
        (P_HPP_ts, P_curtailment_ts, P_charge_discharge_ts, E_SOC_ts, 
         Unserved_A_ts, Shortfall_B_ts, penalty_ts) = ems_cplex_offgrid_full_horizon(
            ...
            it_capacity_mw = it_capacity_mw # PASS THE LIMIT
        )
...
# -------------------------------------------------------------------------
# 2. FULL HORIZON SOLVER (With IT Capacity Constraint)
# -------------------------------------------------------------------------
def ems_cplex_offgrid_full_horizon(
    ...
    for i, t in enumerate(time):
        ...
        # 1. HPP Output = Served Load
        mdl.add_constraint(P_HPP_t[t] == (val_tier_a[i] - U_tier_A[t]) + P_tier_B[t])
        ...
        # 3. IT CAPACITY LIMIT (Prevents Tier B Dumping)
        # Served Tier A + Served Tier B <= IT Capacity
        # Note: U_tier_A is unserved, so Served_A = val_tier_a[i] - U_tier_A[t]
        mdl.add_constraint(
            (val_tier_a[i] - U_tier_A[t]) + P_tier_B[t] <= it_capacity_mw
        )
    ...
    obj_A = mdl.sum(U_tier_A[t] for t in time) * cost_A
    obj_B = mdl.sum(Shortfall_B_daily[d] for d in unique_days) * cost_B
    obj_Curt = mdl.sum(P_curtailment_t[t] for t in time) * cost_Curt
    obj_Deg = mdl.sum((P_charge[t] + P_discharge[t]) for t in time) * cost_Deg

    mdl.minimize(obj_A + obj_B + obj_Curt + obj_Deg)
    ...
    if sol is None:
        ...
        return (np.zeros(len(time)), np.zeros(len(time)), np.zeros(len(time)), 
                np.zeros(len(time)+1), tier_a_profile, np.zeros(len(time)), np.zeros(len(time)))
    ...
    Penalty_res = U_A_res * cost_A + S_B_res * cost_B 
    mdl.end()
    
    return P_HPP_res, P_curt_res, P_batt_res, E_SOC_res, U_A_res, S_B_res, Penalty_res
